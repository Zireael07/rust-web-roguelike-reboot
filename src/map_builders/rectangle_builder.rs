...
impl RectBuilder {
    ...
    fn build(&mut self, _rng : &mut RandomNumberGenerator, build_data : &mut BuilderMap) {
        ...
        let max_x = if rect.0 + rect.3 < build_data.map.width { rect.0 + rect.3 } else { build_data.map.width };
    ...
    fn num_unbroken_floors_columns(&self, build_data : &mut BuilderMap) -> Vec<Vec<i32>> {
        ...
        for x in 0 .. build_data.map.width as usize {
            for y in 0 .. build_data.map.height as usize {
                ...
                num_floors[x][y] = if build_data.map.tiles[idx] == TileType::Floor { 1 + add } else {0};
            }
        }
