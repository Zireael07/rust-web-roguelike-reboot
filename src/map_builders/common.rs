...
pub fn paint(map: &mut Map, mode: Symmetry, brush_size: i32, x: i32, y:i32) {
    match mode {
        Symmetry::None => apply_paint(map, brush_size, x, y, TileType::Floor),
        Symmetry::Horizontal => {
            let center_x = map.width / 2;
            if x == center_x {
                apply_paint(map, brush_size, x, y, TileType::Floor);                    
            } else {
                let dist_x = i32::abs(center_x - x);
                apply_paint(map, brush_size, center_x + dist_x, y, TileType::Floor);
                apply_paint(map, brush_size, center_x - dist_x, y, TileType::Floor);
            }
        }
        Symmetry::Vertical => {
            ...
            if y == center_y {
                ...
                apply_paint(map, brush_size, x, center_y + dist_y, TileType::Floor);
                apply_paint(map, brush_size, x, center_y - dist_y, TileType::Floor);
            }
        }
        Symmetry::Both => {
